...
# This pipeline is reused for all products
try:
    classifier = pipeline(
...
def classify_text(text: str) -> dict:
    ...
    try:
        result = classifier(
            ...
            truncation=True,
            max_length=512,
            multi_label=True  # Enables multi-label mode as per documentation
        )
        # The result returns labels and scores, map them to a dictionary
...
def process_product_reviews(product_slug: str):
    ...
    # Add new columns for scores (normalized label names for convenience)
    for label in LABELS:
        # Replace non-alphanumeric characters with underscore for column safety
